...
/* XXX no 'addr' field present in GDB 7
  if (os)
	vma += os->addr;
 */
  ...
  if (rs->buf[0] == 'E')
	error ("target memory fault, section %s, range 0x%s -- 0x%s",
	       ...
	       core_addr_to_string (vma),
	       core_addr_to_string (vma + size));
  if (rs->buf[0] != 'C')
	{
...
struct saa_    *sfaa = arg;
  ...
  sfaa->of = symbol_file_add_from_bfd (sfaa->sbfd, sfaa->ftty, sfaa->sa, 0);
#if 0
...
static char*
rtems_pid_to_str (struct target_ops *ops, ptid_t ptid)
{
  static char buf[30];

  sprintf(buf, "Thread 0x%08x",
	(unsigned)(ptid_is_pid (ptid) ? ptid_get_pid (ptid) : ptid_get_tid (ptid)));
  return buf;
}
...
static void rtems_remote_open(char *name, int from_tty)
{
  ...
  remote_open_1 (name, from_tty, &rtems_ops, 0);
  if ( exec_bfd )
	{
	  asection *s;
  ...
  /* Executing stuff on the target is broken and currently not supported */
  set_gdbarch_push_dummy_call(target_gdbarch, NULL);
}

static void rtems_files_info(struct target_ops *t)
{
struct objfile *of;
unsigned long   vma, sz;
sec_ptr         s;
  ...
  ALL_OBJFILES(of)
	{
	  if ( of->obfd )
		{
		  printf_filtered("%s:\n", bfd_get_filename( of->obfd ));
		  if ( info_verbose )
			{
			  for ( s=of->obfd->sections; s; s=s->next )
				{
				  if (    (SEC_ALLOC & bfd_get_section_flags (of->obfd, s))
				       && (sz = bfd_get_section_size( s )) > 0 )
					{
					  vma = (unsigned long)bfd_get_section_vma (of->obfd, s);
					  printf_filtered("  0x%08lx - 0x%08lx", vma, vma + sz - 1);
					}
				  else
					{
					  puts_filtered("    -------- -   --------");
					}
				  printf_filtered(": %s\n", bfd_get_section_name( of->obfd, s ));
				}
			}
		}
	  else
...
static void
rtems_detach(struct target_ops *ops, char *args, int from_tty)
{
	remote_detach(ops, args, from_tty);
	pop_target();
}
*/
...
static void
change_obj_stub(struct objfile *obj)
{
  if ( obj && obj == symfile_objfile )
  {
	if ( lookup_minimal_symbol("_Thread_Handler", NULL, NULL) )
	{
...
{
#if 0 /* 2010/3/16 - This might be ineffective and should probably done when
       * opening the target. Depending on the order of execution of the initializers
       * the change made here is reverted by gdbarch initialization.
       */
  ...
  /* 
   * Register a callback to override the 'main name' every time a new
   * main executable is loaded.
   */
  observer_attach_new_objfile( change_obj_stub );
